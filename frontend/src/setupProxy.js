 ...
 *     - if (fs.existsSync(paths.proxySetup)) { ... } 구문에서 setupProxy.js 
 *          파일이 존재하는지 체크함
 *     - 이때, proxySetup은 node_modules/react-scripts/config/paths.js 
 *          파일에서 정의된 proxySetup 변수임
...
module.exports = function(app){
    /* SpringBoot 서버 간의 프록시(Proxy) 설정 정의 */
    ...
    app.use(
        ...
        createProxyMiddleware({
            ...
            // 위에 설정된 속성값을 이용하여 -> URL 경로를 재정의
            // - URL : http://localhost:3000/spring/react/xxx
            
            // - 1차 서버주소로 변경 
            //    : http://localhost:5000/flask/react/xxx

            // - 2차 /spring 제거하기
            //    : "^/flask" -> 변경할 부분을 찾는 정규표현식
            //                 -> ^ 뒤에 문자열 부분을 찾아서
            //                 -> : "" 공백으로 치환(바꿔치기, 제거 의미를 가짐)
            //    : http://localhost:5000/react/xxx            
            pathRewrite : {
                "^/flask" : "",
            },
        })
